...
def get_pipeline_health() -> Optional[dict]:
    ...
    has passed since it started (run_pipeline.py is meant to fire every
    15-30 min once scheduled, so a longer gap means the scheduler itself
    has likely stopped firing)."""
    supabase = get_client()
    resp = (
